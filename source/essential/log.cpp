					...
					if (entry_data) {
						...
						for (auto &current_stream : used_streams) {
							...
							for (auto const &chunk : entry_data->chunks) {
								if (current_stream.should_be_written(chunk, entry_data->type)) {
									if (!was_header_printed) {
										current_stream.write(header(std::move(entry_data->tags),
																	current_stream.are_tags_enabled()).c_str());
										was_header_printed = true;
									}
									current_stream.write(chunk.value.c_str());
									current_stream.write("\n");
								}
							}
							if (was_header_printed) {
								current_stream.write(u8"\n");

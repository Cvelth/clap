...
void window::initialize() {
	...
	text.push_back(
		{
			std::make_unique<clap::render::text>(
				u8"A simple example. 日本語もここだよ!",
	...
	counter_text = --text.end();

	text.push_back(
		{
			std::make_unique<clap::render::text>(
				"Mouse cursor is hovering over the window!",
				clap::resource::font["Kalam-Light"],
				text_program, 24
			), int(width() - 450), int(height() / 2 - 70)
		});
	hover_text = --text.end();
...
void window::render() {
	...
	static size_t drawn_counter = -1;

	poll_events();
	if (should_update()) {
		clear::color();

		if (drawn_counter != counter_value) {
			counter_text->text->update(std::to_string(counter_value));
			drawn_counter = counter_value;
		}

		for (auto iterator = text.begin(); iterator != text.end(); iterator++)
			if (iterator != hover_text)
				iterator->text->draw(iterator->x, iterator->y);
			else
				if (hover_value)
					hover_text->text->draw(hover_text->x, hover_text->y);
...
bool window::on_mouse_hover(clap::event::mouse_hover_action action) {
	if (action == clap::event::mouse_hover_action::enter)
		hover_value = true;
	else
		hover_value = false;
	update();
